...
BEGIN_MESSAGE_MAP(CMainFrame, CProtocolFrame)
	...
	ON_UPDATE_COMMAND_UI(ID_FB12_COUNTER, OnUpdateCounter)
...
static UINT indicators_run[] =
{
	...
	ID_FB12_COUNTER,		// count up timer
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	bool run_mode = (MKGetApp()->m_mode == CMKApp::APPMODE_RUN) ? true : false;
...
void CMainFrame::OnUpdateViewRunbar(CCmdUI* pCmdUI)
{
	...
	pCmdUI->Enable(MKGetApp()->m_mode == CMKApp::APPMODE_RUN);
...
void CMainFrame::OnUpdateCounter(CCmdUI* pCmdUI)
{
	// TODO: Add your command update UI handler code here
	CMKDoc* pDoc = MKGetDoc();
	if (pDoc)
	{
		pCmdUI->SetText(pDoc->GetCounterText());
	}
}
...
{
	if (MKGetApp()->m_pFB12Device)
	{
		if (MKGetApp()->m_pFB12Device->get_dooropen() == VARIANT_TRUE)
		{
			MKGetApp()->m_pFB12Device->SetDoorOpen(VARIANT_FALSE);
		}
		else
		{
			MKGetApp()->m_pFB12Device->SetDoorOpen(VARIANT_TRUE);
		}
	}
}
...
void CMainFrame::OnUpdateDoor(CCmdUI* pCmdUI)
{
	if (MKGetApp()->m_pFB12Device)
	{
		pCmdUI->SetCheck(MKGetApp()->m_pFB12Device->get_dooropen() ? 1 : 0);
	}
}
#endif
