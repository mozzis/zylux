...
void CMainFrame::OnUpdateViewRunbar(CCmdUI* pCmdUI)
{
	...
	pCmdUI->Enable(DAGetApp()->m_mode == CDAApp::APPMODE_RUN);
...
{
	if (DAGetApp()->m_pFB12Device)
	{
		DAGetApp()->m_pFB12Device->SetDoorOpen(!DAGetApp()->m_pFB12Device->get_dooropen());
	}
}
...
void CMainFrame::OnUpdateDoor(CCmdUI* pCmdUI)
{
	if (DAGetApp()->m_pFB12Device)
	{
		pCmdUI->SetCheck(DAGetApp()->m_pFB12Device->get_dooropen() ? 1 : 0);
	}
}
#endif
