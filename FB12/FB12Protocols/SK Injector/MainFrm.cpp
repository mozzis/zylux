...
BEGIN_MESSAGE_MAP(CMainFrame, CProtocolFrame)
	...
	ON_COMMAND(ID_VIEW_INJECTBAR, OnViewInjectbar)
	ON_UPDATE_COMMAND_UI(ID_VIEW_INJECTBAR, OnUpdateViewInjectbar)
	//}}AFX_MSG_MAP
...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	// Create m_wndInjectorBar
	if (!m_wndInjectorBar.Create(this, WS_CHILD | WS_VISIBLE | CBRS_TOP |
			CBRS_TOOLTIPS | CBRS_FLYBY, IDW_INJECTBAR) ||
		!m_wndInjectorBar.LoadToolBar(IDR_INJECTOR))
	{
		TRACE0("Failed to create injector toolbar\n");
		return -1;
	}
	...
	m_wndInjectorBar.SetWindowText(_T("Injector"));
	m_wndInjectorBar.EnableDocking(CBRS_ALIGN_ANY | CBRS_FLOAT_MULTI);
	...
	DockControlBarRightOf(&m_wndInjectorBar, &m_wndRunBar);
	DockControlBarRightOf(&m_wndFB12Bar, &m_wndInjectorBar);

	LoadBarState(_T("Toolbars"));

	
	ShowControlBar(&m_wndRunBar, run_mode ? TRUE : FALSE, FALSE);
	ShowControlBar(&m_wndInjectorBar, ((run_mode && (SKGetApp()->m_skoptions.m_num_injectors > 0)) ? TRUE : FALSE), FALSE);
...
void CMainFrame::OnUpdateTimer(CCmdUI* pCmdUI)
{
	...
	if (pDoc)
	{
		pCmdUI->SetText(pDoc->GetTimerText());
	}
}
...
void CMainFrame::OnViewInjectbar() 
{
	// TODO: Add your command handler code here
	bool bVisible = ((m_wndInjectorBar.GetStyle() & WS_VISIBLE) != 0);
	ShowControlBar(&m_wndInjectorBar, !bVisible, FALSE);
	RecalcLayout();	
}

void CMainFrame::OnUpdateViewInjectbar(CCmdUI* pCmdUI) 
{
	// TODO: Add your command update UI handler code here
	pCmdUI->Enable((SKGetApp()->m_mode == CSKApp::APPMODE_RUN) && 
					(SKGetApp()->m_skoptions.m_num_injectors > 0));
	pCmdUI->SetCheck((m_wndInjectorBar.GetStyle() & WS_VISIBLE) != 0);	
}
