...
void CSample::SetReplicate(int replicate, long RLU)
{
...
void CSample::ClearReplicate(int replicate)
{
	if (replicate < m_replicates.GetSize())
	{
...
void CSample::Recalculate()
{
	...
	if (num > 0)
	{
		...
		m_net = ave - m_background;			// Set the net
