...
void CRepInfoList::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) 
{
		...
		(lpDrawItemStruct->itemAction & (ODA_DRAWENTIRE | ODA_SELECT)))
	{
		...
		if (GetCheck(item) == 1)
		{
			...
			{
				SCFormatDecisecondsCh(rep_info.m_peaktime, dsec_str);
				pDC->ExtTextOut(rect.left,
					rect.top + max(0, (cyItem - m_cyText) / 2),
					ETO_OPAQUE, &(rect), dsec_str,
					strlen(dsec_str), NULL);
			}
			...
			{
				SCFormatDecisecondsCh(rep_info.m_halfrise, dsec_str);
				pDC->ExtTextOut(rect.left,
					rect.top + max(0, (cyItem - m_cyText) / 2),
					ETO_OPAQUE, &(rect), dsec_str,
					strlen(dsec_str), NULL);
			...
			{
				SCFormatDecisecondsCh(rep_info.m_halfdecay, dsec_str);
				pDC->ExtTextOut(rect.left,
					rect.top + max(0, (cyItem - m_cyText) / 2),
					ETO_OPAQUE, &(rect), dsec_str,
					strlen(dsec_str), NULL);
