...
{
	TCHAR dsec_str[32];
	SCFormatDecisecondsCh(m_pps.UseStartButton() ? 0 : m_pps.m_delaytime, dsec_str);
	SetDlgItemText(IDC_EDIT_DELAY, dsec_str);
}

void CSKEditDlg::SetMeasText()
{
	TCHAR dsec_str[32];
	SCFormatDecisecondsCh(m_pps.m_meastime, dsec_str);
	SetDlgItemText(IDC_EDIT_MEAS, dsec_str);
}

void CSKEditDlg::SetIntervalText()
{
	TCHAR dsec_str[32];
	SCFormatDecisecondsCh(m_pps.m_intervaltime, dsec_str);
	SetDlgItemText(IDC_EDIT_INTERVAL, dsec_str);
}

void CSKEditDlg::SetDurationText()
{
	TCHAR dsec_str[32];
	SCFormatDecisecondsCh(m_duration, dsec_str);
	SetDlgItemText(IDC_EDIT_DURATION, dsec_str);
}
