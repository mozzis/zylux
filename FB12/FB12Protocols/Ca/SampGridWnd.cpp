// SampGridWnd.cpp : implementation file
//
...
void CSampGridWnd::SetGridValues(CCAPPS& pps)
{
	UINT i;
		
	ChangeStandardStyle(CGXStyle().SetMaxLength(pps.SAMPLENAME_SIZE - 1));

	SetRowCount(pps.m_numsamps);


	// Only set the list entries into the grid.
	for (i=1; i<=pps.m_numsamps; i++)
	{
		SetValueRange(CGXRange(i, 1), LPCTSTR(pps.m_samplelist[i-1]));
	}


}

void CSampGridWnd::GetGridValues(CCAPPS& pps)
{
	ROWCOL i;
