...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	bool run_mode = (QMGetApp()->m_mode == CQMApp::APPMODE_RUN) ? true : false;
	...
	if (!m_wndFB12Bar.Create(this, WS_CHILD | WS_VISIBLE | CBRS_TOP |
		CBRS_TOOLTIPS | CBRS_FLYBY, ID_VIEW_FB12) ||
	...
	DockControlBarRightOf(&m_wndFB12Bar, &m_wndToolBar);
...
void CMainFrame::OnUpdateTimer(CCmdUI* pCmdUI)
{
	// TODO: Add your command update UI handler code here
	CString txt;
	CQMDoc* pDoc = (CQMDoc*)QMGetApp()->m_pDoc;
	if (pDoc)
	{
...
{
	if (QMGetApp()->m_pFB12Device)
	{
		QMGetApp()->m_pFB12Device->SetDoorOpen(!QMGetApp()->m_pFB12Device->get_dooropen());
	}
}
...
void CMainFrame::OnUpdateDoor(CCmdUI* pCmdUI)
{
	if (QMGetApp()->m_pFB12Device)
	{
		pCmdUI->SetCheck(QMGetApp()->m_pFB12Device->get_dooropen() ? 1 : 0);
	}
}
#endif
