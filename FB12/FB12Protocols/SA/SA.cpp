...
BEGIN_MESSAGE_MAP(CSAApp, CProtocolApp)
	//{{AFX_MSG_MAP(CSAApp)
...
CSAApp::CSAApp()  : CProtocolApp(IDR_MAINFRAME, IDR_SATYPE, IDR_KINETICSFRAME)
{
...
BOOL CSAApp::ModeRunView()
{
	...
	m_grid_template = new CMultiDocTemplate(
		(m_mode == APPMODE_RUN) ? IDR_SATYPE : IDR_SAVIEWTYPE,
...
BOOL CSAApp::ModeEdit()
{
	...
	CSAEditDlg dlg(m_pps, m_filetitle);
