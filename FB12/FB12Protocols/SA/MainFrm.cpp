...
int CMainFrame::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	bool run_mode = (SAGetApp()->m_mode == CSAApp::APPMODE_RUN) ? true : false;
...
void CMainFrame::OnUpdateViewRunbar(CCmdUI* pCmdUI)
{
	...
	pCmdUI->Enable(SAGetApp()->m_mode == CSAApp::APPMODE_RUN);
...
{
	if (SAGetApp()->m_pFB12Device)
	{
		SAGetApp()->m_pFB12Device->SetDoorOpen(!SAGetApp()->m_pFB12Device->get_dooropen());
	}
}
...
void CMainFrame::OnUpdateDoor(CCmdUI* pCmdUI)
{
	if (SAGetApp()->m_pFB12Device)
	{
		pCmdUI->SetCheck(SAGetApp()->m_pFB12Device->get_dooropen() ? 1 : 0);
	}
}
#endif
