...
CString CProtocolGridWnd::SendToExcel97(LPCTSTR macroname, LPCTSTR workbookname,
		LPCTSTR worksheetname, WORD cell_col, WORD cell_row)
{
	...
	try
	{
		...
		char ole_str[] = "Excel.Application.8";
		try
		{
		...
		{
			CString startup_path((LPCTSTR)pXL->StartupPath);
			CFileFind finder;
			...
			while (bWorking)
			{
				...
				{
					SetFocus();
					CString err_str("Unable to open your startup workbook in Excel (TM) 97.\n\n");
		...
		if (workbook_valid)
		{
			...
			if (!book_found)
			{
				...
				{
					SetFocus();
					CString err_str("Unable to open workbook in Excel (TM) 97.\n\n");
		...
		if (!book_found)
		{
			...
			pBook = pBooks->Add((long)xlWBATWorksheet);
			book_found = true;
			if (create_book)
			{
				_variant_t opt((long)DISP_E_PARAMNOTFOUND, VT_ERROR);
				pBook->SaveAs(workbookname, opt, opt, opt, opt, opt,
					xlNoChange, opt, opt, opt, opt);
			}
		}
		...
		if (book_found)
		{
			...
			if (worksheet_valid)
			{
				...
				if (!sheet_found)
				{
					...
					{
						SetFocus();
						CString err_str("Unable to create worksheet in Excel (TM) 97.\n\n");
			...
			if (!sheet_found)
			{
				// Get a handle to the first Worksheet in Workbook
				pSheet = pSheets->Item[1L];
				sheet_used = true;
				sheet_found = true;
			}
			...
			if (sheet_found)
			{
				...
				if (psa)
				{
					...
					if (SUCCEEDED(SafeArrayAccessData(psa, (void **)&data)))
					{
						...
						pXL->WindowState = xlNormal;

						// Activate the sheet
						pSheet->Activate();

						// Send the array to Excel
						pSheet->Range[(LPCTSTR)rng_str]->Value = varr;
						...
						if (macro_valid)
						{
							...
							{
								SetFocus();
								error_str = "An error occurred while executing a macro in Excel (TM) 97.";
								error_str += "\n\nError Message:\n";
					...
					{
						error_str = "Excel (TM) 97 setup problem:\n\nCould not get access to array data.";
					}
	...
	{
		error_str = "Error communicating with Excel (TM) 97.";
		error_str += "\n\nError Message:\n";
		error_str += (LPCTSTR)e.ErrorMessage();
		if (e.Description().length() > 0)
		{
